...
const { MatrixClient, MatrixAuth, SimpleFsStorageProvider, AutojoinRoomsMixin, RustSdkCryptoStorageProvider, } = require("matrix-bot-sdk");
...
const run = async () => {
    const storageDirectory = path_1.default.join(process.env.HOME, ".matrix-bot", "storage");
    mkdirp_1.default.sync(storageDirectory);
    const storage = new SimpleFsStorageProvider(path_1.default.join(storageDirectory, "simple.json"));
    const cryptoStorage = new RustSdkCryptoStorageProvider(storageDirectory);
    const _client = await new MatrixAuth("https://" + process.env.MATRIX_HOMESERVER).passwordLogin(process.env.MATRIX_USERNAME, process.env.MATRIX_PASSWORD, process.env.MATRIX_USERNAME);
    client = await new MatrixClient("https://" + process.env.MATRIX_HOMESERVER, _client.accessToken, storage, cryptoStorage);
    ...
    client.on("room.message", async (roomId, event) => {
        ...
        if (event["content"]?.["msgtype"] !== "m.text")
            return; //don't repond to non-text messages
